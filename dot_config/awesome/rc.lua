...
globalkeys = mytable.join(
    ...
    function()
        awful.spawn("rofi -show-icons -sidebar-mode -show drun -terminal alacritty -theme ~/.config/rofi/config.rasi")
    end,
    ...
    awful.key({ modkey }, "F11", 
        function()
            ...
            awful.util.spawn_with_shell("if [ $(brightnessctl g) -lt $(( $(brightnessctl m) / 10 )) ]; then brightnessctl set 10%; fi")
        end),

    awful.key({ modkey }, "F12", 
        function()
            awful.util.spawn_with_shell("brightnessctl set +10% -n 1")
            awful.util.spawn_with_shell("if [ $(brightnessctl g) -gt $(brightnessctl m) ]; then brightnessctl set 100%; fi")
        end),
